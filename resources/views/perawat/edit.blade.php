	...
	<div class="container">
		<div class="col-md-12">
			<div class="panel panel-primary">
			  <div class="panel-heading">Edit Data Perawat
			  	...
			  	<form action="{{ route('perawat.update',$perawat->id) }}" method="post" >
        			...
        			{{ csrf_field() }}
			  		<div class="form-group {{ $errors->has('nama') ? ' has-error' : '' }}">
			  			...
			  			<input type="text" name="nama" value="{{ $perawat->nama }}" class="form-control"  required>
			  			@if ($errors->has('nama'))
			  		...
			  		<div class="form-group {{ $errors->has('pasien_id') ? ' has-error' : '' }}">
			  			...
			  			<select name="pasien_id" class="form-control">
			  				@foreach($mhs as $data)
			  				<option value="{{ $data->id }}" {{ $selectedperawat == $data->id ? 'selected="selected"' : '' }} >{{ $data->nama }}</option>
			  				@endforeach
			  			</select>
